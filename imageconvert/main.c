...
int main(int argc, const char * argv[]) {
    ...
    if (pBMP[0] == '/' && pBMP[1] == '/' && pBMP[2] == 0x0a) {
        iSize = ParseHeader(pBMP, iSize);
        if (!iSize) { // invalid input
    ...
    if (*(uint16_t *)pBMP == BB_BITMAP_MARKER) { // we're converting a G5 to a BMP
    ...
    } else if (*(uint16_t *)pBMP == BB_BITMAP2_MARKER) { // 2-bit (separate planes
        pBBBM = (BB_BITMAP *)pBMP;
        w = pBBBM->width;
        h = pBBBM->height;
        ...
        iSize = iPitch * h; // offset to second plane
        for (y=0; y<h; y++) {
            uint8_t uc0, uc1;
            int x;
            s = &pOut[y * iPitch];
            ...
            for (x=0; x<w; x++) {
                if ((x & 7) == 0) {
                    uc0 = s[0]; uc1 = s[iSize]; // pixels from each bit plane
                    s++;
                }
                if (x & 1) {
                    d[0] |= ((uc0 >> 7) & 1) | ((uc1 >> 6) & 2);
        ...
        SaveBMP(argv[2], &pOut[(iPitch * h * 2)], ucBWYRPalette, w, h, 4);
    } else {
